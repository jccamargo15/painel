var startRender = function() {
	...
	if (!Function.prototype.bind) {
		Function.prototype.bind = function(object) { // eslint-disable-line
														// no-extend-native
			var self = this, slice = Array.prototype.slice, args = slice.call(
					arguments, 1);
			return function() {
	...
	var queryUtil = (function() {
		...
		return {
			...
			getParameterByName : getParameterFromQuery.bind(null,
					window.location.search),
			isParameterTrue : isParameterTrueInQuery.bind(null,
					window.location.search)
		};
	}());
	...
	function PackagesUtil(packagesStructure, query) {
		...
		this.getVersionsByQuery = function(versionsObject) {
			...
			if (packages.all) {
				Object.keys(versionsObject)
						.forEach(
								applyVersion.bind(null,
										getVersionString(packages.all)));
			}
		...
		this.buildConfig = function(config) {
			var debug = (queryParamsObject.debug || '').split(',').filter(
					Boolean);
			if (debug.indexOf('all') !== -1) {
				var debuggableExternals = Object.keys(config.paths).filter(
						function(path) {
					...
					.forEach(
							function(k) {
								if (typeof config.paths[k] === 'object'
										&& !(config.paths[k] instanceof Array)) {
									config.paths[k] = config.paths[k][isInDebug(k) ? 'source'
											: 'min'];
								}
							});
			...
			packagesStructure.filter(function(pkg) {
				return !isInDebug(pkg);
			...
			var projectPackages = packagesStructure.filter(isInDebug).map(
					function(name) {
						return {
							name : name,
	...
	// //////////////////////////////////////////////////////////////////////
	var packagesUtil = new PackagesUtil([ "animations", "cloud", "components",
			"componentsPreviewLayer", "core", "documentServices",
			"editingRendererPlugins", "fonts", "imageClientApi", "layout",
			"previewExtensionsCore", "qaAutomation", "render", "server",
			"skins", "testUtils", "tpa", "tweenEngine", "utils", "widgets",
			"wixCode", "wixSites", "wixappsBuilder", "wixappsClassics",
			"wixappsCore" ], window.location.search);
	...
	function replaceUrlVersion(url, version) {
		return url && onlySemver.test(version) ? url.replace(semverRegex,
				version) : url;
	}

	function overrideScriptsLocationMapFromQuery(scriptsLocationMap,
			overrideParam) {
		overrideParam.split(',').filter(Boolean).forEach(
				function(keyValueString) {
					var pair = keyValueString.split(':');
					var urlOverride = replaceUrlVersion(
							scriptsLocationMap[pair[0]], pair[1]);
					if (urlOverride
	...
	function instrument(serviceTopology, wixBiSession, queryUtil, siteModel) {
		wixBiSession.initialTimestamp = wixBiSession.initialTimestamp
				|| wixBiSession.mainLoaded;
		...
		var isPremium = !!premiumFeatures
				&& premiumFeatures.indexOf('HasDomain') !== -1;
		var base = (serviceTopology.biServerUrl || 'http://frog.wix.com')
				.replace(/\/$/, '');
		...
		function prepareMessage(evid, src, options) {
			...
			if (!omit.ts && wixBiSession.initialTimestamp) {
				msg += param('ts')
						+ (Date.now() - wixBiSession.initialTimestamp);
			}
		...
		wixBiSession.sendBI = function(endpoint, evid, code, extra, sampleRatio) {
			if (!sampleRatio
					|| Math.floor(wixBiSession.random * sampleRatio) === 0) {
		...
		var sendError = function(name, code, severity) {
			sendError = function() {
			}; // only report one error per session

			var extra = '&errn=' + encodeURIComponent(name) + '&errc=' + code
					+ '&sev=' + severity + '&errscp=core&cat=2&iss=1&et='
					+ wixBiSession.et;

			var total = 0;
			var params = Array.prototype.slice.call(arguments, 3).map(
					function(arg, index) {
		...
		wixBiSession.sendError = function(err) {
			sendError.apply(null,
					[ err.errorName, err.errorCode, err.severity ]
		...
		if (newrelic) {
			...
			if (newrelic.setCustomAttribute) {
				...
				if (rendererModel) {
					if (rendererModel.siteInfo && rendererModel.siteInfo.siteId) {
						newrelic.setCustomAttribute('sid',
								rendererModel.siteInfo.siteId);
					}
					if (rendererModel.metaSiteId) {
						newrelic.setCustomAttribute('msid',
								rendererModel.metaSiteId);
					}
					var runningExperiments = rendererModel.runningExperiments;
					if (runningExperiments) {
						runningExperiments = Object.keys(runningExperiments)
								.filter(function(key) {
									return this[key] === 'new';
								}, runningExperiments).join('][');
						if (runningExperiments) {
							newrelic.setCustomAttribute('experiments', '['
									+ runningExperiments + ']');
						}
					}
					newrelic.setCustomAttribute('preview',
							rendererModel.previewMode ? 1 : 0);
				}
				...
				if (publicModel && publicModel.timeSincePublish) {
					newrelic.setCustomAttribute('timeSincePublish',
							publicModel.timeSincePublish);
				}
				if (wixBiSession.viewerSessionId) {
					newrelic.setCustomAttribute('vsi',
		...
		if (!wixBiSession.viewerSessionId || rendererModel.previewMode
		...
		var isBot = /bot|google|crawler|spider|robot|crawling|^Mozilla\/4\.0$/i
		...
		(function validityBI() {
			...
			var extra = boolParam('top', isTop) + boolParam('bot', isBot)
					+ boolParam('sbot', isSuspectBot);
			wixBiSession.sendBI('ugc-viewer', 361, 42, extra,
					sampleRatioState === 'none' ? 0 : 10);
		}());
		...
		(function performanceBI() {
			...
			if (timing) {
				...
				var loadTime = timing.navigationStart || timing.fetchStart
						|| timing.domainLookupStart || timing.connectStart;
		...
		var ignoreURLs = [ /^chrome(\-extension)?\:/, /^file\:/, /^resource\:/,
				/\.net\//, /\.info\//, /\.ru\//, /google/, /facebook/,
				/dropbox/, /ad\-score/, /drivemac/, /shopping/, /datafast/,
				/shopcomp/, /vimeo/, /olark/ ];
		...
		window.onerror = function(errorMsg, url, line, column, err) {
			...
			if (!ignoreError(where)) {
				sendError('JAVASCRIPT_ERROR', 111022, 40, errorMsg, where,
						line, column); // JAVASCRIPT_ERROR from
										// packages/core/src/main/bi/errors.js
			}
		...
		if (window.console) {
			...
			if (origError) {
				console.error = function() {
					sendError.bind(null, 'CONSOLE_ERROR', 111023, 30).apply(
							null, arguments); // CONSOLE_ERROR from
												// packages/core/src/main/bi/errors.js
		...
		requirejs.onError = function(err) {
			...
			sendError(errn, errc, severity, err.message, modules, where); // REQUIREJS_ERROR
																			// from
																			// packages/core/src/main/bi/errors.js
		};
	}
	...
	function prefetchPages(siteModel, publicModel) {
		...
		if (window.pagesData && window.pagesData.masterPage) {
			return contentCache; // don't pre fetch when pages data was part
									// of server side rendered response
		}
		...
		var prefetchLangs = preloadSupported() ? function(json) {
			doPrefetchLangs(json.data.document_data.SITE_STRUCTURE.characterSets);
		}
				: function() {
				};
		...
		function doPrefetchLangs(langs) {
			if (langs) {
				var baseUrl = /localhost|127.0.0.\d/.test(siteModel.santaBase) ? siteModel.serviceTopology.scriptsLocationMap.santa
						: siteModel.santaBase;
				langs.forEach(function(lang) {
					var link = document.createElement('link');
					link.setAttribute('rel', 'preload');
					link.setAttribute('href', joinURL(baseUrl,
							'static/css/user-site-fonts', lang + '.css'));
	...
	function convertSiteModel(rendererModel, publicModel) {
		...
		siteModel.siteHeader = {
			id : siteModel.rendererModel.siteId,
			userId : siteModel.rendererModel.userId
		}; // required
		siteModel.siteId = siteModel.rendererModel.siteId; // required
		siteModel.viewMode = siteModel.rendererModel.previewMode ? 'preview'
				: 'site'; // required
	...
	function movePageDataToMaster(siteAsJson) {
		...
		siteAsJson.pages
				.forEach(function(page) {
					...
					if (pageItem && pageItem.pageBackgrounds
							&& pageItem.pageBackgrounds.desktop.ref) {
						// BackgroundImage or BackgroundMedia
						desktopBg = get(pageData,
								pageItem.pageBackgrounds.desktop.ref);
						mobileBg = get(pageData,
								pageItem.pageBackgrounds.mobile.ref);
						move(pageItem.pageBackgrounds.desktop.ref, masterData,
								pageData);
						move(pageItem.pageBackgrounds.mobile.ref, masterData,
								pageData);
						...
						if (desktopBg) {
							if (desktopBg.mediaRef) {
								moveMediaRef(masterData, pageData,
										desktopBg.mediaRef);
							}
							// Image
	 ...
	 * 
	 * sagi: modified to handle all mobile devices (especially fixes android's
	 * firefox and IE on win phones)
	 * 
	 */
	function fixViewport(siteModel) {
		function isMobileDevice() {
			...
			return patternByDevice.test(userAgent)
					|| patternByModel.test(userAgent.substr(0, 4));
		}
		...
		if (isMobileDevice() && isOptimizedForMobileSite()) {
			...
			document
					.write('<meta id="wixMobileViewport" name="viewport" content="width=321, user-scalable=no, maximum-scale=2.2">');
		}
	}
	/* global joinURL */
	function getViewerRjsConfig(serviceTopology) {
		...
		var serviceURL = joinURL.bind(null, scriptsLocation, 'services',
				'third-party');
		...
		return {
			...
			paths : {
				...
				react : {
					min : serviceURL('react/0.14.3/react-with-addons.min'),
					source : '//cdnjs.cloudflare.com/ajax/libs/react/0.14.3/react-with-addons'
				},
				reactDOM : {
					min : serviceURL('react/0.14.3/react-dom.min'),
					source : '//cdnjs.cloudflare.com/ajax/libs/react/0.14.3/react-dom'
				},
				reactDOMServer : {
					min : serviceURL('react/0.14.3/react-dom-server.min'),
					source : '//cdnjs.cloudflare.com/ajax/libs/react/0.14.3/react-dom-server'
				},
				zepto : serviceURL('zepto/1.1.3/zepto.min'),
				speakingurl : serviceURL('speakingurl/speakingurl.min'),
				immutable : {
					min : serviceURL('immutable/3.6.2/immutable.min'),
					source : serviceURL('immutable/3.6.2/immutable')
				},
				mousetrap : serviceURL('mousetrap/1.4.6/mousetrap.min'),
				swfobject : serviceURL('swfobject/2.3.20130521/swfobject.min'),
				TweenMax : {
					min : serviceURL('tweenmax/1.18.2/minified/TweenMax.min'),
					source : serviceURL('tweenmax/1.18.2/uncompressed/TweenMax')
				},
				TimelineMax : {
					min : serviceURL('tweenmax/1.18.2/minified/TweenMax.min'),
					source : serviceURL('tweenmax/1.18.2/uncompressed/TweenMax')
				...
				ScrollToPlugin : {
					min : serviceURL('tweenmax/1.18.2/minified/plugins/ScrollToPlugin.min'),
					source : serviceURL('tweenmax/1.18.2/uncompressed/plugins/ScrollToPlugin')
				},
				DrawSVGPlugin : {
					min : serviceURL('tweenmax/1.18.2/minified/plugins/DrawSVGPlugin.min'),
					source : serviceURL('tweenmax/1.18.2/uncompressed/plugins/DrawSVGPlugin')
				},
				...
				bluebird : {
					min : serviceURL('bluebird/2.9.4/bluebird.min'),
					source : serviceURL('bluebird/2.9.4/bluebird')
				},
				...
				jsonpatch : {
					min : 'js/vendor/json-patch/json-patch-duplex.min',
					source : 'js/vendor/json-patch/json-patch-duplex'
				}
			},
			// generated
			packages : [
					{
						name : 'tpaIntegration',
						main : 'tpaIntegration',
						location : getIntegrationPath()
								+ '/tpaIntegration/src/main'
					},
					{
						name : 'wixCodeIntegration',
						main : 'wixCodeIntegration',
						location : getIntegrationPath()
								+ '/wixCodeIntegration/src/main'
					} ],
			bundles : null,
			shim : {
				zepto : {
	...
	function getFullRjsConfig(rjsConfigFunc, packagesUtil, artifactData,
			serviceTopology) {

		// Call with serviceTopology and all arguments after
		var config = rjsConfigFunc.apply(null, Array.prototype.slice.call(
				arguments, 3));

		config = packagesUtil.buildConfig(config);
		var isAddress = RegExp.prototype.test.bind(/^https?:\/\//);

		var artifactPath = joinURL(serviceTopology.scriptsDomainUrl,
				'services', artifactData.artifactName);
		if (artifactData.baseVersionOverride) {
			config.baseUrl = isAddress(artifactData.baseVersionOverride) ? artifactData.baseVersionOverride
	...
	function getSubdomain(domain) {
		...
		if (subDomain.length <= 2) {
			...
			if (topLevelDomains[beforeLastPart]) {
				subDomain = subDomain[subDomain.length - 3] + '.'
						+ subDomain[subDomain.length - 2] + '.'
						+ subDomain[subDomain.length - 1];
			} else {
				subDomain = subDomain[subDomain.length - 2] + '.'
						+ subDomain[subDomain.length - 1];
			}
		}
		return subDomain;
	...
	function render(isServerSide, isPreview, contentCache, queryUtil,
			siteModel, wixBiSession) {

		var performanceNow = performance.now ? performance.now
				.bind(performance) : Date.now.bind(Date);
		...
		function addConditionalDependencies(pkgs) {
			...
			var isQaAutomation = queryUtil.isParameterTrue.bind(queryUtil,
					'isqa');
			var isTpaIntegration = queryUtil.isParameterTrue.bind(queryUtil,
					'isTpaIntegration');
			var isWixCodeIntegration = queryUtil.isParameterTrue.bind(
					queryUtil, 'isWixCodeIntegration');
			...
			if (isTpaIntegration()) {
				pkgs.push('tpaIntegration', 'jasmine', 'jasmine-html',
						'bluebird');
			}
			if (isWixCodeIntegration()) {
				pkgs.push('wixCodeIntegration', 'jasmine', 'jasmine-html',
						'bluebird');
			}
			if (isWixCloud()) {
				pkgs.push('cloud');
			...
			if (isPreview || shouldLoadPackageFor('wixapps')
					|| shouldLoadPackageFor('ecommerce')) {
		...
		function load(pkgs, callback) {
			/*
			 * Require all needed packages (static + conditional) Then do
			 * initial site render (or re-layout if it was rendered by the
			 * server)
			 */
			function getAjaxHandler($) {
				...
				var async = window.Promise && window.Promise.resolve ? (function() {
				...
				function simResponse(content, options) {
					var success = options.success.bind(options.context
							|| window, content);
				...
				return function ajax(options) {
					...
					if (options.cache !== false
							&& (cachedContent = contentCache[options.url])
							&& canUseCache(options)) {
			...
			requirejs(pkgs, function() {
				...
				function initConditionalDependencies(_pkgs) {
					...
					if (_pkgs.wixCodeIntegration) {
						_pkgs.wixCodeIntegration
								.init(window, p.core, p.wixCode);
					}
				}
				...
				// Wait for DOM to be ready before accessing it, e.g.
				// getElementById
				p.zepto(function() {
					if (siteModel.wixData) {
						var siteStructureNode = document
								.getElementById('SITE_STRUCTURE');
						siteModel.wixHtmlRaw = siteStructureNode.outerHTML;
						siteModel.wixAnchors = window.anchors || {};
						siteStructureNode.parentNode
								.removeChild(siteStructureNode);
					}
					...
					siteModel.currentUrl = p.utils.urlUtils
		...
		function renderClientSide() {
			// because ajv loads js-beautify synchronously even when does not
			// use it
			define('js-beautify', {}); // eslint-disable-line
										// santa/module-definition
			var clientSidePackages = [ 'skins', 'components', 'utils', 'core',
					'react', 'lodash', 'TimelineMax', 'layout', 'tpa', 'zepto',
					'fonts', 'color', 'animations', 'imageClientApi',
					'swfobject', 'mousetrap', 'tweenEngine', 'DrawSVGPlugin',
					'reactDOM', 'ScrollToPlugin', 'widgets', 'experiment',
					'js-beautify', 'render' ];
			if (queryUtil.isParameterTrue('ds') || isPreview) {
				clientSidePackages.push('documentServices',
						'componentsPreviewLayer');
			}
			load(clientSidePackages, function(loadedPackages, ajaxHandler) {
				loadedPackages.render.clientSide(loadedPackages, ajaxHandler,
						siteModel/* , onReady */);
			});
		}
		...
		function renderServerSide() {
			load(
					[ 'layout', 'utils', 'zepto', 'lodash', 'fonts', 'color' ],
					function(p, ajaxHandler) {
						var getDomNode = function() {
							var domId = p.lodash.toArray(arguments).join('');
							return document.getElementById(domId);
						};
						var siteData = new p.utils.SiteData(siteModel,
								ajaxHandler);
						siteData.currentPageInfo = p.utils.wixUrlParser
								.parseUrl(siteData, siteData.currentUrl.full);
						var currentPage = siteData.currentPageInfo.pageId;
						var requests = p.utils.pageRequests(siteData,
								siteData.currentPageInfo);
						siteData.store
								.loadBatch(
										requests,
										function() {
											...
											p.layout.reLayout(structuresDesc,
													siteData);
											...
											window.sssr.serverSideRender = {
												sinceInitialTimestamp : (Date
														.now() - window.wixBiSession.initialTimestamp),
	...
	function addExperimentsFromQuery(runningExperiments, queryUtil) {
		function overrideFromParam(experiments, param, value) {
			return queryUtil.getParameterByName(param).split(',').filter(
					Boolean).reduce(function(accum, exp) {
				accum[exp] = value;
				return accum;
			}, experiments);
		}

		var experiments = Object.keys(runningExperiments).reduce(
				function(accum, exp) {
	...
	/*
	 * global packagesUtil, instrument, queryUtil,
	 * overrideScriptsLocationMapFromQuery, prefetchPages, convertSiteModel,
	 * fixViewport, getFullRjsConfig, getViewerRjsConfig, getSubdomain, render,
	 * convertRendererModel, joinURL, addExperimentsFromQuery
	 */
	...
	serviceTopology.scriptsLocationMap = overrideScriptsLocationMapFromQuery(
			serviceTopology.scriptsLocationMap, queryUtil
					.getParameterByName('scriptsLocations'));
	window.rendererModel.runningExperiments = addExperimentsFromQuery(
			window.rendererModel.runningExperiments, queryUtil);
	...
	if (window.karmaIntegration) {
		siteModel.documentServicesModel = siteModel.documentServicesModel
				|| window.karmaIntegration.documentServicesModel;
	...
	serviceTopology.staticServerFallbackUrl = serviceTopology.staticServerFallbackUrl
			|| 'https://fallback.wix.com/';
	var semverMatches = siteModel.santaBase.match(/\/(\d+\.\d+\.\d+)\/?$/);
	siteModel.baseVersion = semverMatches && semverMatches[1];
	siteModel.santaBaseFallbackUrl = siteModel.baseVersion ? joinURL(
			serviceTopology.staticServerFallbackUrl, siteModel.baseVersion)
			: null;
	...
	render(isServerSide, isPreview, contentCache, queryUtil, siteModel,
			wixBiSession);
};
startRender();